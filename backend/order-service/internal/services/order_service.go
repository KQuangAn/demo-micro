...
func (s *OrderService) HandleOrderProcessingNotificationSentEvent(ctx context.Context, id string) (*model.Order, error) {
	tx, err := s.repo.BeginTransaction(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to begin transaction: %v", err)
	}
	...
	order.Status = model.OrderStatus(model.OrderStatusCompleted.String())
	...
	event := &ebTypes.PutEventsRequestEntry{
		Source:       aws.String(os.Getenv("EVENT_BRIDGE_EVENT_SOURCE")),
		DetailType:   aws.String(enums.EVENT_TYPE.OrderCompleted.String()),
		Detail:       aws.String(string(detailJSON)),
		EventBusName: aws.String(os.Getenv("EVENT_BRIDGE_BUS_NAME")),
	}
